  ...
  <div>
    ...
    <div class="m_newadd_content">
      <div class="m_order_nav">
        ...
        <span id="optionText">新建</span>收货地址
      </div>
      <!--标题-->
      <div class="m_new_add">
        <div class="love_name">
          <ul>
            ...
            <li class="love_dq" @click="popup()">所在地区
              <span id="addspan" style="padding-left: 30px;color:#414141">{{address}}</span>
            </li>

            <li class="add_xx">详细地址
              <input id="address"  v-model="detailAddress" maxlength="100" class="add_xx_txt" placeholder="请输入详细地址">
            </li>
      ...
      <div class="sendmobile-box">
        <div class="sendmobile" style="width:90%; margin:0 auto; border-top:1px solid #EFEFEF ">
          订货人手机 <input id="userMobile" v-model="orderPhone" type="text" style="width:60%;display: inline-block; color:#414141" disabled="disabled">
        </div>
      </div>
      <!--收获人手机-->
      <div class="userinfo" style="margin-top: 28%">
        ...
        <div class="new_add_btn">
          ...
          <div class="new_add_user">
            <input id="btnNewAddrSave" @click="privateAddAddress" type="button" style="background-color: #e1e1e1" value="保存并使用">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      privateAddAddress:function () {//新增收货地址信息
        if(this.name==""||this.name.length<0){
          ...
          return false;
        }  if(this.detailAddress==""||this.detailAddress.length<0){
          Toast.fail({
            message:"请填写详细地址",
          })
          return false;
        ...
        //console.log(this.uno,this.name,this.phone,this.domit,this.city,this.area,this.detailAddress,this.email);
        $.ajax({
          url:URL+"DeliveryAddress/addAddress",
          type:"post",
          data:{cdno:this.uno,cdname:this.name,cdphone:this.phone,cdomit:this.domit,
            cdcity:this.city,cdarea:this.area,cdaddress:this.detailAddress,cemail:this.email},
        }).done(function (data) {
          if(data.code==1){
            Toast.success({
              message:"添加成功",
      ...
      getUserInfo:function(){//获取用户相关信息
         this.uno=JSON.parse(localStorage.getItem("user")).cuno;//获取用户编号
          this.orderPhone=JSON.parse(localStorage.getItem("user")).cutel;//获取用户登录的手机号
      ...
      confirm:function (arr) {//获取收货地址并关闭弹层
        ...
        this.address=areaList[0]+"  "+areaList[1]+"  "+areaList[2];//使用地址弹出层之后获得地址
        this.detailAddress=areaList[0]+""+areaList[1]+""+areaList[2];//地址选中之后方便用户
        this.show=false;
      },
